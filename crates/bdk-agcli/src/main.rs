...
fn init_command() -> Command {
    Command::new(
        "init",
        "Generate or import a wallet mnemonic and descriptors",
    )
...
fn address_command() -> Command {
    Command::new("address", "Get next unused receive address")
        ...
        .sync_handler(|_req: &CommandRequest<'_>, _ctx: &mut ExecutionContext| {
            let config = resolve_config();
...
fn sync_command() -> Command {
    Command::new("sync", "Sync wallet with Esplora")
        ...
        .handler(|_req: &CommandRequest<'_>, _ctx: &mut ExecutionContext| {
            Box::pin(async move {
                let config = resolve_config();
                let (mut db, mut wallet) = bdk_lib::load_wallet(&config).map_err(err)?;
    ...
    Command::new("balance", "Show wallet balance")
        ...
        .sync_handler(|_req: &CommandRequest<'_>, _ctx: &mut ExecutionContext| {
            ...
            Ok(CommandOutput::new(json!(balance)).next_actions(vec![
                ...
                NextAction::new("bdk-agcli send --to <addr> --amount <sats>", "Send BTC"),
            ]))
        })
}
        ...
        .handler(|req: &CommandRequest<'_>, _ctx: &mut ExecutionContext| {
            ...
            Box::pin(async move {
                let to = to.ok_or_else(|| {
                    CommandError::new("--to is required", "MISSING_ARG", "Provide --to=<address>")
                })?;
                ...
                let result =
                    bdk_lib::send(&mut wallet, &mut db, &config.esplora_url, &to, amount_sats)
                        .await
                        .map_err(err)?;
